...
//Implementation of Regulafalsi function
...
//for loop for interval 1 to check for roots.
...
//Implementation of be=rute force sign change approach
...
//Printing MACD and signal to output file for excel to plot 
...
//Calculating the 26 days EMA using EMA expansion for close price
...
//Calculating the 12 day EMA using EMA expansion for close price
	for(int t=0;t<size;t++){
...
//Calculating 9 day EMA over MACD line using EMA expansion 
	for(int t=0;t<size;t++){
...
//Assigning the macd and signal to global variables for easy access
	macd=macd_line;
	signal=t9emaTable;
	
	//Calling brute force or regulafalsi
	builtTradePoints();
...
//returning values from signal table
double table_signal_position(long unsigned int timeidx)
{
...
//Calculating and returning the linnear interpolated values for MACD
double macd_position(double time)
{
...
//Calculating and returning the linnear interpolated values for Signal
double signal_position(double time)
{
